<template>
  <div class="student-task-view">
    ...
    <n-modal v-model:show="showTaskDetail" preset="card" :title="currentTask?.title || '任务详情'" style="width: 500px">
      <div v-if="currentTask && currentSubmission" class="task-detail-modal">
        ...
        <n-divider title="教师评语" v-if="taskComment" />
        <div v-if="taskComment" class="task-comment">
          <n-tag size="small" :type="getCommentType(taskComment.type)">
            {{ getCommentTypeLabel(taskComment.type) }}
          </n-tag>
          <p>{{ taskComment.content }}</p>
        </div>
      </div>
    </n-modal>
  </div>
...
const pendingTasks = computed((): Task[] => {
  return studentTasks.value.filter((task: Task) => {
    const submission = teacherStore.getSubmissionsByStudent(currentStudent.value!.id)
      .find(s => s.taskId === task.id)
    return !submission || !submission.isCompleted
  })
})
...
const taskComment = computed(() => {
  if (!currentTask.value || !currentStudent.value) return null
  return teacherStore.getCommentsByStudent(currentStudent.value.id)
    .find(c => c.taskId === currentTask.value!.id)
})
